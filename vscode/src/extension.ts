...
export function activate(context: vscode.ExtensionContext) {
  ...
  // ─── Status Bar: Workspace Health ─────────────────────────────────
  const healthBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 99);
  ...
  function updateHealthBar() {
    ...
    try {
      ...
      const done = objects.filter((o: any) => o.status === '[x]' || o.status === 'done').length;
      const blocked = objects.filter((o: any) => o.status === '[!]' || o.status === 'blocked').length;
      if (blocked > 0) {
        healthBar.text = `$(warning) ALP: ${total} objs · ${blocked} blocked`;
        healthBar.backgroundColor = new vscode.ThemeColor('statusBarItem.warningBackground');
      } else if (total === 0) {
      ...
      healthBar.backgroundColor = new vscode.ThemeColor('statusBarItem.errorBackground');
    }
  }

  vscode.window.onDidChangeActiveTextEditor(updateHealthBar, null, context.subscriptions);
  vscode.workspace.onDidSaveTextDocument(updateHealthBar, null, context.subscriptions);
  updateHealthBar();
...
<body>
  ...
  const copyCmd = vscode.commands.registerCommand('alp.copyObject', async () => {
    ...
    const updated = lines.map((line) => {
      ...
      if (stripped.startsWith('id:')) {
        const match = stripped.match(/^id:\s*(.+)$/);
        if (match && match[1].trim() === sourceId) {
          replacements += 1;
      ...
      if (updateRefs === 'Yes') {
        for (const field of refFields) {
          if (stripped.startsWith(`${field}:`)) {
            ...
            if (refMatch && refMatch[1].trim() === sourceId) {
              replacements += 1;
