...
<body>
  ...
  const visualizeCmd = vscode.commands.registerCommand('alp.visualizeWorkflow', async () => {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
      vscode.window.showWarningMessage('Open an ALP file first.');
      return;
    ...
    const objects = getParsedObjects(editor.document);
    const workflows = objects.filter((o: any) => o._type === 'workflow');
    if (workflows.length === 0) {
      vscode.window.showInformationMessage('No @workflow objects found to visualize.');
      return;
    }

    const panel = vscode.window.createWebviewPanel('alpVisualize', 'ALP Workflow Visualization', vscode.ViewColumn.One, {});
    panel.webview.html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><style>
  body { font-family: var(--vscode-font-family); padding: 16px; color: var(--vscode-foreground); }
...
<body>
  <h2>Workflow Visualization (${workflows.length} workflow(s))</h2>
  <pre>graph TD\n  ${workflows.map((w: any) => `  ${w.id || 'unknown'}`).join('\n  ')}</pre>
</body></html>`;
  });

  const graphCmd = vscode.commands.registerCommand('alp.showGraph', () => {
    const editor = vscode.window.activeTextEditor;
    const objects = editor ? getParsedObjects(editor.document) : [];
    const nodes = objects.filter((o: any) => o._type === 'task');
    const edges: string[] = [];
    ...
    const panel = vscode.window.createWebviewPanel('alpGraph', 'ALP Dependency Graph', vscode.ViewColumn.One, {});
    panel.webview.html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><style>
  body { font-family: var(--vscode-font-family); padding: 16px; color: var(--vscode-foreground); }
...
<body>
  <h2>Dependency Graph (${nodes.length} nodes, ${edges.length} edges)</h2>
  <pre>graph TD\n${edges.map((e) => `  ${e}`).join('\n')}</pre>
</body></html>`;
  });

  const statusCmd = vscode.commands.registerCommand('alp.showStatus', () => {
    const editor = vscode.window.activeTextEditor;
    const objects = editor ? getParsedObjects(editor.document) : [];
    const stats: Record<string, { total: number; done: number; inProgress: number; todo: number; blocked: number }> = {
      project: { total: 0, done: 0, inProgress: 0, todo: 0, blocked: 0 },
      feature: { total: 0, done: 0, inProgress: 0, todo: 0, blocked: 0 },
      task: { total: 0, done: 0, inProgress: 0, todo: 0, blocked: 0 },
      workflow: { total: 0, done: 0, inProgress: 0, todo: 0, blocked: 0 },
    };
    for (const obj of objects) {
    ...
    const items = Object.entries(stats)
      .filter(([, s]) => s.total > 0)
      .map(([type, s]) => `${type.toUpperCase()}S (${s.total} total)\n  [x] Done: ${s.done}\n  [~] In Progress: ${s.inProgress}\n  [ ] Todo: ${s.todo}\n  [!] Blocked: ${s.blocked}`)
      .join('\n\n');
    const panel = vscode.window.createWebviewPanel('alpStatus', 'ALP Project Status', vscode.ViewColumn.One, {});
    panel.webview.html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><style>
  body { font-family: var(--vscode-font-family); padding: 16px; color: var(--vscode-foreground); }
...
<body>
  ...
  <pre>${escapeHtml(items || '(no status data)')}</pre>
</body></html>`;
  });

  context.subscriptions.push(visualizerCmd, policyCmd, timelinesCmd, policiesCmd, contractsCmd, vaultsCmd, agentsCmd, diffCmd, renameCmd, copyCmd, statsCmd, templateCmd, moveCmd, searchCmd, formatCmd, lintCmd, testCmd, visualizeCmd, graphCmd, statusCmd, archiveCmd, promoteCmd);
