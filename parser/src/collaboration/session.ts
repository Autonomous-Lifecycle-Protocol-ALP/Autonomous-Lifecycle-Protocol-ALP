/**
 * ALP Collaboration — session, operation, branching, and presence management.
 */
...
export class SessionManager {
  ...
  createSession(docId: string, initialState?: Record<string, any>): CollabSession {
    ...
    this.activity.logActivity(docId, 'agent_run', 'system', { action: 'session_created' });
    return session;
  }
  ...
  joinSession(docId: string, agentId: string): PresenceInfo | null {
    ...
    this.activity.logActivity(docId, 'team_edit', agentId, { action: 'joined_session' });
    return presence;
  }
  ...
  applyOperation(
    ...
    if (!this.permissions.checkPermission(docId, agentId, 'edit')) {
      return null;
    }
    ...
    this.activity.logActivity(docId, 'team_edit', agentId, { action: type, path });
    this.audit.appendAudit(agentId, 'collab:apply_operation', docId, { opId: op.id, path, type });
  ...
  fork(docId: string, branchId: string): CollabBranch | null {
    ...
    this.activity.logActivity(docId, 'branch', 'system', { action: 'fork', branchId });
    this.audit.appendAudit('system', 'collab:fork', docId, { branchId });
    return branch;
  }
  ...
  mergeBranch(docId: string, branchId: string): MergeResult | null {
    ...
    this.activity.logActivity(docId, 'merge', 'system', { action: 'merge', branchId, conflicts: conflicts.length });
    this.audit.appendAudit('system', 'collab:merge', docId, { branchId, opsApplied, conflicts: conflicts.length });
  ...
  setOpCounter(n: number): void {
    this.opCounter = n;
  }

  serializeSessions(): any[] {
    return Array.from(this.sessions.entries()).map(([docId, session]) => ({
      docId,
  ...
  restoreSessions(entries: any[]): void {
