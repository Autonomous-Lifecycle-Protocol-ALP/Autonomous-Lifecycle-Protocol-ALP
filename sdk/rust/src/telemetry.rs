...
impl Span {
    pub fn new(id: impl Into<String>, trace_id: impl Into<String>, span_id: impl Into<String>, parent_span_id: Option<String>, agent: Option<String>, action: impl Into<String>, start_time: i64, status: impl Into<String>, attributes: HashMap<String, serde_json::Value>) -> Self {
        Self {
            id: id.into(),
...
impl TelemetryEngine {
    ...
    pub fn end_span(&mut self, span_id: &str, status: impl Into<String>, attributes: Option<HashMap<String, serde_json::Value>>) -> Option<Span> {
    ...
    pub fn export_otlp(&self) -> HashMap<String, serde_json::Value> {
        ...
        for s in &self.completed_spans {
            ...
            span_map.insert("traceId".into(), serde_json::Value::String(s.trace_id.clone()));
            span_map.insert("spanId".into(), serde_json::Value::String(s.span_id.clone()));
            span_map.insert("parentSpanId".into(), serde_json::Value::String(s.parent_span_id.clone().unwrap_or_default()));
            span_map.insert("name".into(), serde_json::Value::String(s.action.clone()));
            span_map.insert("kind".into(), serde_json::Value::Number(1.into()));
            span_map.insert("startTimeUnixNano".into(), serde_json::Value::String(format!("{}", s.start_time * 1000000)));
            let end_time = s.end_time.unwrap_or(s.start_time);
            span_map.insert("endTimeUnixNano".into(), serde_json::Value::String(format!("{}", end_time * 1000000)));
            ...
            let attrs: Vec<HashMap<String, serde_json::Value>> = s.attributes.iter().map(|(k, v)| {
                let mut attr = HashMap::new();
                attr.insert("key".into(), serde_json::Value::String(k.clone()));
                attr.insert("value".into(), serde_json::json!({"stringValue": v.to_string()}));
                attr
            }).collect();
            span_map.insert("attributes".into(), serde_json::Value::Array(attrs.into_iter().map(|a| serde_json::Value::Object(a.into_iter().map(|(k, v)| (k, v)).collect())).collect()));
            spans.push(serde_json::Value::Object(span_map.into_iter().map(|(k, v)| (k, v)).collect()));
        }
        let scope = HashMap::from([
            ("name".into(), serde_json::Value::String("@autonomous-lifecycle-protocol-alp/telemetry".into())),
            ("version".into(), serde_json::Value::String("17.0.0".into())),
        ]);
        let scope_span = HashMap::from([
            ("scope".into(), serde_json::Value::Object(scope.into_iter().map(|(k, v)| (k, v)).collect())),
            ("spans".into(), serde_json::Value::Array(spans)),
        ]);
        scope_spans.push(serde_json::Value::Object(scope_span.into_iter().map(|(k, v)| (k, v)).collect()));

        let resource_attrs = vec![
            HashMap::from([
                ("key".into(), serde_json::Value::String("service.name".into())),
                ("value".into(), serde_json::json!({"stringValue": "alp-execution-engine"})),
            ]),
            HashMap::from([
                ("key".into(), serde_json::Value::String("telemetry.sdk.name".into())),
                ("value".into(), serde_json::json!({"stringValue": "alp-telemetry"})),
            ]),
        ];
        let resource = HashMap::from([
            ("attributes".into(), serde_json::Value::Array(resource_attrs.into_iter().map(|a| serde_json::Value::Object(a.into_iter().map(|(k, v)| (k, v)).collect())).collect())),
        ]);
        let resource_span = HashMap::from([
            ("resource".into(), serde_json::Value::Object(resource.into_iter().map(|(k, v)| (k, v)).collect())),
            ("scopeSpans".into(), serde_json::Value::Array(scope_spans)),
        ]);

        let mut otlp = HashMap::new();
        otlp.insert("resourceSpans".into(), serde_json::Value::Array(vec![serde_json::Value::Object(resource_span.into_iter().map(|(k, v)| (k, v)).collect())]));
        otlp
    }
    ...
    pub fn get_trace_summary(&self) -> HashMap<String, usize> {
        ...
        summary.insert("totalSpans".into(), self.completed_spans.len() + self.active_spans.len());
