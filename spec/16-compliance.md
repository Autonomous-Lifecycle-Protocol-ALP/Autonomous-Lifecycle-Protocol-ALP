...
The **ALP Compliance Test Suite** provides a standardized set of test fixtures to ensure that custom parser implementations conform to the ALP specification.
...
- `!if`/`!assert` directives evaluate against the current object's context.
...
3. **Error Category** (e.g., `SyntaxError`, `IndentationError`, `ValidationError`, `DirectiveError`).
...
- **Unknown Directives (v8.0.0+):** An unrecognised directive (e.g. `!asret: ...`) MUST throw a `SyntaxError`. Forward compatibility applies only to *known* directives; an unknown name is a hard syntax violation.
- **Failed `!assert` (v8.0.0+):** A `!assert` whose ALPEL expression is false, OR whose expression fails to evaluate, MUST throw a `DirectiveError`. `!assert` is fail-closed — evaluation errors are no longer silently ignored.
- **Status Marker Deprecation (v8.0.0+):** A `status: [!]` or `status: [?]` without a trailing reason text MUST emit a deprecation warning. Required (hard error) in v9.0.0.
