...
| `38.0.0` | 2026-07-22 | Memory Mesh Engine. `MemoryMeshEngine` enables cross-agent semantic memory sharing with decay scoring, memory node relationships, and distributed knowledge graph queries. New `parser/src/memory-mesh.ts` and `sdk/python/alp_sdk/memory_mesh.py`; CLI `alp memory-mesh`; tests in `parser/tests/memory-mesh.test.ts` and `sdk/python/tests/test_memory_mesh.py`. |
| `37.0.0` | 2026-07-22 | Macro Engine & Collaboration Engine. `MacroEngine` provides dynamic `@macro` object generation with parameterized templates; `CollaborationEngine` adds real-time multiplayer conflict resolution with operational transforms, presence tracking, and branch synchronization. New `parser/src/macro.ts`, `parser/src/collaboration.ts`, `sdk/python/alp_sdk/macro.py`, `sdk/python/alp_sdk/collaboration.py`; CLI `alp macro` and `alp collab`; tests in `parser/tests/macro.test.ts`, `parser/tests/collaboration.test.ts`, and Python parity suites. |
| `36.0.0` | 2026-07-22 | Autonomous Swarm Marketplace & Skill Registry. `SwarmMarketplaceEngine` adds agent skill registration, category discovery, skill invocation, rating, cost-per-call tracking, and audit logging. New `parser/src/swarm-marketplace.ts` and `sdk/python/alp_sdk/swarm_marketplace.py`; CLI `alp marketplace`; schema `swarm_marketplace.schema.json`. |
| `35.0.0` | 2026-07-22 | Decoupled Event Mesh. `EventMeshEngine` provides asynchronous pub/sub event mesh topic routing and payload broadcasting across swarms. New `parser/src/event-mesh.ts` and `sdk/python/alp_sdk/event_mesh.py`; CLI `alp event-mesh`; schema `event_mesh.schema.json`. |
| `34.0.0` | 2026-07-22 | Code Transformation & Refactoring. `CodeTransformEngine` performs automated AST refactoring and source code migration rules. New `parser/src/code-transform.ts` and `sdk/python/alp_sdk/code_transform.py`; CLI `alp code-transform`; schema `code_transform.schema.json`. |
| `33.0.0` | 2026-07-22 | Consensus Voting Protocol. `ConsensusVoteEngine` supports multi-agent proposal creation, voting algorithms (majority, supermajority, weighted), and tallying. New `parser/src/consensus-vote.ts` and `sdk/python/alp_sdk/consensus_vote.py`; CLI `alp consensus-vote`; schema `consensus_vote.schema.json`. |
| `32.0.0` | 2026-07-22 | Automated Prompt Optimizer. `PromptOptimizerEngine` performs prompt compression, instruction tuning, and token efficiency optimization. New `parser/src/prompt-optimizer.ts` and `sdk/python/alp_sdk/prompt_optimizer.py`; CLI `alp prompt-optimizer`; schema `prompt_optimizer.schema.json`. |
| `31.0.0` | 2026-07-22 | Model Evaluation Suite. `EvalSuiteEngine` adds benchmark case evaluation, output assertions, and regression testing. New `parser/src/eval-suite.ts` and `sdk/python/alp_sdk/eval_suite.py`; CLI `alp eval-suite`; schema `eval_suite.schema.json`. |
| `30.0.0` | 2026-07-22 | Code Symbol Indexer. `CodeIndexEngine` performs AST code symbol indexing, function mapping, and dependency searching. New `parser/src/code-index.ts` and `sdk/python/alp_sdk/code_index.py`; CLI `alp code-index`; schema `code_index.schema.json`. |
| `29.0.0` | 2026-07-22 | Edge Model Routing. `EdgeModelEngine` provides edge-native LLM routing, latency-based selection, and model execution budgeting. New `parser/src/edge-model.ts` and `sdk/python/alp_sdk/edge_model.py`; CLI `alp edge-model`; schema `edge_model.schema.json`. |
| `28.0.0` | 2026-07-21 | Vector Store (`@vector_store`). `VectorStoreEngine` adds vector similarity search and semantic retrieval. New `parser/src/vector-store.ts` and `sdk/python/alp_sdk/vector_store.py`; schema `vector_store.schema.json`. |
| `27.0.0` | 2026-07-21 | Sandbox Environment (`@sandbox_env`). `SandboxEnvEngine` provides containerized sandboxing and permission enforcement. New `parser/src/sandbox.ts` and `sdk/python/alp_sdk/sandbox_env.py`; schema `sandbox_env.schema.json`. |
| `26.0.0` | 2026-07-21 | CRDT Distributed Sync (`@crdt_sync`). `CRDTSyncEngine` enables distributed multi-node CRDT state synchronization. New `parser/src/crdt-distributed.ts` and `sdk/python/alp_sdk/crdt_sync.py`; schema `crdt_sync.schema.json`. |
| `25.0.0` | 2026-07-21 | Cost Budget (`@cost_budget`). `CostBudgetEngine` enforces hard-stop token spending limits and financial budgets. New `parser/src/cost-budget.ts` and `sdk/python/alp_sdk/cost_budget.py`; schema `cost_budget.schema.json`. |
| `24.0.0` | 2026-07-21 | Asset Context (`@asset_context`). `AssetContextEngine` handles workspace asset linking and context binding. New `parser/src/asset-context.ts` and `sdk/python/alp_sdk/asset_context.py`; schema `asset_context.schema.json`. |
| `23.0.0` | 2026-07-21 | Architecture Decomposer (`@arch_decomposer`). `ArchDecomposerEngine` performs system architecture decomposition into tasks/workflows. New `parser/src/arch-decomposer.ts` and `sdk/python/alp_sdk/arch_decomposer.py`; schema `arch_decomposer.schema.json`. |
| `22.0.0` | 2026-07-21 | Self Healing (`@self_healing`). `SelfHealingEngine` provides automatic patch generation and workspace repair. New `parser/src/healing.ts` and `sdk/python/alp_sdk/self_healing.py`; schema `self_heal.schema.json`. |
| `21.0.0` | 2026-07-21 | Anomaly Detection (`@anomaly`). `AnomalyDetectorEngine` adds statistical runtime behavior anomaly detection. New `parser/src/anomaly.ts` and `sdk/python/alp_sdk/anomaly.py`; schema added. |
| `20.0.0` | 2026-07-21 | Cost Optimizer (`@cost_optimizer`). `CostOptimizerEngine` provides predictive token and execution cost reduction. New `parser/src/cost-optimizer.ts` and `sdk/python/alp_sdk/cost_optimizer.py`; schema added. |
| `19.0.0` | 2026-07-21 | Resilience Engine (`@resilience`). `ResilienceEngine` adds fault-tolerant execution, circuit breakers, and retries. New `parser/src/resilience.ts` and `sdk/python/alp_sdk/resilience.py`; schema added. |
| `18.4.0` | 2026-07-21 | Cross-Domain Trust (`@domain_trust`). `DomainTrustAnchor` bootstraps trust between sovereign domains via signed trust-root exchange. New `sdk/python/alp_sdk/domain_trust.py`; tests in `sdk/python/tests/test_domain_trust.py`. |
| `18.3.0` | 2026-07-21 | Autonomous Governance (`@governance`). `GovernanceEngine` collects signed policy ballot votes and enforces quorum rules. New `sdk/python/alp_sdk/governance.py`; tests in `sdk/python/tests/test_governance.py`. |
| `18.2.0` | 2026-07-21 | Multi-Tenant Isolation (`@tenant`). `TenantVault` extends `Vault` with cryptographic namespace isolation. New `sdk/python/alp_sdk/tenant.py`; tests in `sdk/python/tests/test_tenant.py`. |
| `18.1.0` | 2026-07-21 | Decentralized Coordination (`@p2p`). `P2PSwarm` replaces central coordinator with DHT discovery and gossip-based state sync. New `sdk/python/alp_sdk/p2p.py`; tests in `sdk/python/tests/test_p2p.py`. |
| `18.0.0` | 2026-07-21 | Self-Sovereign Identity (`@did_identity`). W3C DID-based agent identity: `AgentIdentity` creates/manages DIDs, `IdentityResolver` verifies presentations, `TrustRegistry` maps DIDs to permission scopes. New `sdk/python/alp_sdk/identity.py`; tests in `sdk/python/tests/test_identity.py`. |
| `17.0.0` | 2026-07-21 | Universal Protocol Bridge. `ProtocolBridge` adapts ALP objects to/from OpenAPI 3.0, GraphQL SDL, gRPC proto, and AsyncAPI specs. New `sdk/python/alp_sdk/bridge.py`; tests in `sdk/python/tests/test_bridge.py`. |
| `16.1.0` | 2026-07-21 | MCP Expansion, Enriched Examples & E2E Tests. Four new MCP tools (`alp_check_policy`, `alp_visualize`, `alp_search_registry`, `alp_get_timelines`); three new JSON schemas (`contract`, `vault`, `timeline`); enriched example workspaces; E2E integration test suite (`cli/tests/e2e.test.ts`). Full suite: 376 tests across 49 files, zero failures. |
...
The current release is **v44.0.0** (V11 — IDE Intelligence). Future versions are tracked in `docs/ROADMAP_V17_V43.md` and the versioning spec's roadmap table. Planned focus areas include:
