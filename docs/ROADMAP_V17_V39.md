...
| **v40.0.0** | **IDE Intelligence** | AI-Powered SHAM IDE — Context-Aware Copilot, Smart Refactoring & Live Collaboration | Contextual ALP copilot with spec-aware autocomplete, inline plan validation, and autonomous fix suggestions; semantic rename and `.alp`-aware refactoring tools; real-time multi-user editing with CRDT-backed cursors and presence; persistent workspace memory across sessions; plugin SDK for extensible tool panels; performance profiler for agent execution traces; cloud sync for settings, snippets, and agent configurations |
...
- **v40.0.0 (IDE Intelligence)**: SHAM IDE gains AI-native capabilities without leaving the editor. Contextual copilot reads the active `.alp` spec, recent event mesh, and swarm marketplace context to offer spec-aware autocomplete, inline plan validation, and autonomous fix suggestions. Semantic rename and `.alp`-aware refactoring tools restructure contracts, macros, and workflows safely. Real-time multi-user editing uses CRDT-backed cursors and presence so distributed teams co-author specs. Persistent workspace memory survives restarts and feeds predictive policy suggestions. A plugin SDK exposes extensible tool panels for custom MCP tools, visual workflow builders, and third-party LSP adapters. A built-in performance profiler traces agent execution, policy decisions, and memory operations. Cloud sync encrypts settings, snippets, and agent configurations end-to-end.
