...
- **Autonomous Reasoning Core (V82.0.0):** Run self-reflection critique on `.alp` specs before committing changes via `node cli/dist/index.js reason critique <file> --refine`. Verify cryptographic Merkle reasoning traces via `node cli/dist/index.js reason verify <chainId>`.
- **MCP Prompts & Resources:** Use MCP prompts `alp_delegate_task`, `alp_review_workspace`, `alp_diagnose_failure`, and resources `alp://workspace`, `alp://graph`, `alp://policies`, `alp://events` when operating over MCP.
