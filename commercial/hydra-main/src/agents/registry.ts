import { BasePersona, type AgentPersona, type PersonaExample } from "./base";
...
export class AgentRegistry {
  ...
  listByPermission(permission: string): AgentPersona[] {
    return this.list().filter((p) => p.permissions.includes(permission));
  }

  listByTool(toolId: string): AgentPersona[] {
    return this.list().filter((p) => p.tools.includes(toolId));
  }
